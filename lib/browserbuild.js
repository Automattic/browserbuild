...
Renderer.prototype._render = function(filepath) {
  ...
  else if (coffeescript && // if filepath has one of these extensions:
    ['coffee', 'litcoffee', 'coffee.md'].some(function(ext){
      var re = new RegExp('^(.*)([^/])(\\.' + ext + ')$');
      if (re.test(filepath)) {
        // compile to JavaScript
        source = coffeescript.compile(source, { filename: filepath,
                                                literate: ext == 'litcoffee'
                                                       || ext == 'coffee.md',
                                                header: true });
        // change extension to .js
        filepath = filepath.replace(re, '$1$2.js');
        return true;
      }
    })) {
    // do nothing: `.some` callback has already compiled CoffeeScript above
  }
  else {
    // remove `if node`
    var ignoring = false
